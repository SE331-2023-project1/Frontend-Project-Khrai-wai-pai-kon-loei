...
onMounted(async () => {
  teacher.value = await teacherStore.getTeacherById(userId)
})
...
const { errors, handleSubmit } = useForm({
  ...
  initialValues: {
    id: teacher.value?.user.id || '', // Set a default value if id is not available
    username: teacher.value?.name || '',
    firstName: teacher.value?.user.firstname || '',
    lastName: teacher.value?.user.lastname || '',
